...
namespace ScanApiHelper
{
	...
	class CommandContext: Java.Lang.Object
    {
    ...
    public class DeviceInfo
    {
        ...
        public DeviceInfo(String name, ISktScanDevice device, long type)
        {
            ...
            _suffix = "\n";
            _notification = null;
			_symbologyInfo=new SymbologyInfo[ISktScanSymbology.id.kSktScanSymbologyLastSymbolID];
    ...
    class ScanApiHelper
    {
		...
		private byte _dataConfirmationMode = (byte)ISktScanProperty.values.confirmationMode.kSktScanDataConfirmationModeDevice;
        ...
        public long DoScanAPIReceive()
        {
            ...
            if (_scanApiOpen)
            {
				ISktScanObject[] scanObj = new ISktScanObject[1];
				result = _scanApi.WaitForScanObject(scanObj, 1);
                if (SktScanErrors.SKTSUCCESS(result))
                {
                    if (result == SktScanErrors.ESKT_NOERROR)
                    {
                        closeScanApi = HandleScanObject(scanObj[0]);
        ...
        public void PostSetConfirmationMode(char mode, ICommandContextCallback callback)
        {
            ...
            newScanObj.Property.Byte=(char)mode; // was cast to byte - FIX IT! GLC
        ...
        public void PostSetDataConfirmation(DeviceInfo deviceInfo, ICommandContextCallback callback)
        {
            ...
            newScanObj.Property.Ulong = ((0 << 6) | (ISktScanProperty.values.dataConfirmation.kSktScanDataConfirmationRumbleNone << 4) | (ISktScanProperty.values.dataConfirmation.kSktScanDataConfirmationBeepGood << 2) | ISktScanProperty.values.dataConfirmation.kSktScanDataConfirmationLedGreen);
        ...
        public void PostStartDecode(DeviceInfo deviceInfo, ICommandContextCallback callback)
        {
            ...
            newScanObj.Property.Type = ISktScanProperty.types.kSktScanPropTypeByte;
            newScanObj.Property.Byte = (char)ISktScanProperty.values.trigger.kSktScanTriggerStart; // was cast to byte - FIX IT! GLC
        ...
        public void PostGetCapabilitiesDevice(DeviceInfo deviceInfo, ICommandContextCallback callback)
        {
            ...
            newScanObj.Property.Type=ISktScanProperty.types.kSktScanPropTypeByte;
            newScanObj.Property.Byte=(char)ISktScanProperty.values.capabilityGroup.kSktScanCapabilityLocalFunctions; // was cast to byte - FIX IT! GLC
        ...
        public void PostGetDeviceSpecific(DeviceInfo deviceInfo, Byte[] bArray, ICommandContextCallback callback)
        {
			...
			newScanObj.Property.Array.SetValue(System.Text.Encoding.UTF8.GetString(bArray).ToCharArray(), bArray.Length);
            CommandContext command = new CommandContext(false, newScanObj, device, null, callback);
            AddCommand(command);
        }
        ...
        public void PostSetDecodeAction(DeviceInfo device, int decodeVal, ICommandContextCallback callback)
        {
            ...
            newScanObj.Property.ID=ISktScanProperty.propId.kSktScanPropIdLocalDecodeActionDevice;
            newScanObj.Property.Type=ISktScanProperty.types.kSktScanPropTypeByte;
            newScanObj.Property.Byte=(char)(decodeVal & 0xffff); // was cast to byte - FIX IT! GLC
        ...
        {
            bool closeScanApi = false;
            switch (scanObject.Message.ID) // changes Msg to Message - FIX IT! GLC
            {
				//case ISktScanMsg.kSktScanMsgIdDeviceArrival: This needs the I removed, and an uppercase K - FIX IT! GLC
				case SktScanMsg.KSktScanMsgIdDeviceArrival:
                    HandleDeviceArrival(scanObject);
                    break;
                case SktScanMsg.KSktScanMsgIdDeviceRemoval:
                    HandleDeviceRemoval(scanObject);
                    break;
                case SktScanMsg.KSktScanMsgGetComplete:
                case SktScanMsg.KSktScanMsgSetComplete:
                    DoGetOrSetComplete(scanObject);
                    break;
                case SktScanMsg.KSktScanMsgIdTerminate:
                    Debug.MSG(Debug.kLevelTrace, "Receive a Terminate event, ask to close ScanAPI");
                    closeScanApi = true;
                    break;
                case SktScanMsg.KSktScanMsgEvent:
        ...
        {
			// The following 3 commands need Msg changed to Message - FIX IT! GLC
		    String friendlyName=scanObject.Message.DeviceName;
		    String deviceGuid=scanObject.Message.DeviceGuid;
		    long type=scanObject.Message.DeviceType;
		    ISktScanDevice device=SktClassFactory.CreateDeviceInstance(_scanApi); // CREATEDEVICEINSTANCE NEEDS LOWERCASE C - FIX IT! GLC
        ...
        {
		    ISktScanDevice iDevice=scanObject.Message.DeviceInterface; // changed Msg to Message - FIX IT! GLC
        ...
        {
			// changed the following two Msg to Message - FIX IT! GLC
		    ISktScanEvent Event=scanObject.Message.Event;
		    ISktScanDevice iDevice=scanObject.Message.DeviceInterface;
			switch(Event.ID)
		    {
		    case ISktScanEvent.id.kSktScanEventError:
			    if(_notification!=null){
			        _notification.OnError(scanObject.Message.Result, // FIX IT! GLC
			            Event.DataType == ISktScanEvent.types.kSktScanEventDataTypeString ? Event.DataString : null);
			    }
			    break;
        ...
        private long DoGetOrSetComplete(ISktScanObject scanObj)
        {
		    ...
		    if (scanObj != null)
		    {
			    result=scanObj.Message.Result; // FIX IT! GLC
